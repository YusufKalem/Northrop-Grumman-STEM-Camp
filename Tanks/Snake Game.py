...
assert WINDOWHEIGHT % CELLSIZE == 0, "Window width must be multiple of cell size"
CELLWIDTH = int(WINDOWWIDTH/CELLSIZE)
CELLHEIGHT = int(WINDOWHEIGHT/CELLSIZE)

#We are making colors that we want as variables
#             R    G    B
WHITE     = (255, 255, 255)
BLACK     = (  0,   0,   0)
RED       = (255,   0,   0)
...
def runGame():
    ...
    while True: # main game loop
        for event in pygame.event.get():
            if event.type == QUIT:
                terminate()
            #Check here for eating self bug
        ...
        for wormBody in wormCoords[1:]:
            if wormBody['x'] == wormCoords[HEAD]['x'] and wormBody['y'] == wormCoords[HEAD]['y']:
                print(str(wormBody['x']) + " " + str(wormCoords[HEAD]['x']))
                return

        # check if the worm eats an apple
        #sometimes worm doesn't eat the apple
